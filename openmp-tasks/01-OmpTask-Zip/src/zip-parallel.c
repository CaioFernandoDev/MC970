...
int main(int argc, char **argv) {
  ...
  #pragma omp single
  {
    int found = 0; // Flag to stop the loop when the password is found
    for (int i = 0; i < max_password_value && !found; i++) {
      #pragma omp task firstprivate(i, fp, ret, cmd) shared(found)
      {
