...
__global__ void histogram_kernel(float *d_h, PPMImage *d_img, int size) {
  ...
  if (idx < size) {
    ...
    atomicAdd(&private_h[16 * d_img->data[idx].red + 4 * d_img->data[idx].green + d_img->data[idx].blue], 1);
  }
...
double Histogram(PPMImage *image, float *h_h) {
  ...
  cudaMemcpy(d_data, image->data, size * sizeof(PPMPixel), cudaMemcpyHostToDevice);
  cudaMemcpy(&(d_img->data), &d_data, sizeof(PPMPixel *), cudaMemcpyHostToDevice);
